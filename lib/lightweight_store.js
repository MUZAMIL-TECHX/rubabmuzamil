...
const store = {
    ...
    // A small independent store per WhatsApp account. Sharing one message
    // store between sockets causes needless disk writes and cross-account
    // memory growth when many sessions are online.
    createStore(filePath) {
        const scoped = Object.create(this)
        scoped.messages = {}
        scoped.contacts = {}
        scoped.chats = {}
        scoped.storeFile = filePath
        scoped.readFromFile = function () {
            return this.__readFromFile(this.storeFile)
        }
        scoped.writeToFile = function () {
            return this.__writeToFile(this.storeFile)
        }
        scoped.__readFromFile = this.readFromFile
        scoped.__writeToFile = this.writeToFile
        return scoped
    },
