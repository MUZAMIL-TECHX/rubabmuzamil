...
const senderIsSudo = await isSudo(sender, sock);
	if (senderIsSudo) return;

	if (!containsURL(SenderMessage.trim())) return;
	
const antilinkConfig = await getAntilink(jid, 'on', sock);
	...
	try {
		...
		switch (action) {
			...
			case 'warn':
const warningCount = await incrementWarningCount(jid, sender, sock);
				if (warningCount >= WARN_COUNT) {
					await sock.groupParticipantsUpdate(jid, [sender], 'remove');
await resetWarningCount(jid, sender, sock);
					await sock.sendMessage(jid, {
