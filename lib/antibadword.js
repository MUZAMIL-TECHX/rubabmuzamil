...
const { askProxAbdullah } = require('./proxabdullah');

// Load antibadword config
function loadAntibadwordConfig(sock, groupId) {
    const data = readSessionJson(sock, 'userGroupData.json', { antibadword: {} });
    return data.antibadword?.[groupId] || {};
}

async function detectBadwordWithAI(userMessage) {
    try {
        const result = await askProxAbdullah(
            `Kiya is message me gaali, abuse, hate speech ya offensive word hai? Deep search karo. ` +
            `Sirf YES ya NO me jawab do. Message: ${userMessage}`
        );
        const normalized = String(result || '').trim().toLowerCase();
        if (/^(yes|haan|ha|true|bad|contains)\b/.test(normalized)) return true;
        if (/^(no|nahin|nahi|false|clean|safe)\b/.test(normalized)) return false;
    } catch (error) {
        console.error('⚠️ AI antibadword check failed:', error?.message || error);
    }
    return null;
}
...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    ...
    if (match === 'on') {
        const existingConfig = await getAntiBadword(chatId, 'on', sock);
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '*AntiBadword is already enabled for this group*' });
        }
        await setAntiBadword(chatId, 'on', 'delete', sock);
    ...
    if (match.startsWith('set')) {
        ...
        await setAntiBadword(chatId, 'on', action, sock);
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    const config = loadAntibadwordConfig(sock, chatId);
    ...
    const antiBadwordConfig = await getAntiBadword(chatId, 'on', sock);
    ...
    let localContainsBadWord = false;
    ...
    for (const word of messageWords) {
        ...
        if (localContainsBadWord) break;
    }

    // The API is called once per group message. If it is unavailable, retain
    // the original local list as a safe fallback instead of disabling moderation.
    const aiContainsBadWord = await detectBadwordWithAI(userMessage);
    const containsBadWord = aiContainsBadWord === null
        ? localContainsBadWord
        : aiContainsBadWord;
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    const botNumbers = [sock.user?.id, sock.user?.lid]
        .filter(Boolean)
        .map(jidNumber)
        .filter(Boolean);
    const bot = groupMetadata.participants.find(p =>
        botNumbers.includes(jidNumber(p.id)) ||
        p.id === sock.user?.id ||
        p.id === sock.user?.lid
    );
    if (!bot?.admin) {
    ...
    const participant = groupMetadata.participants.find(p =>
        p.id === senderId || jidNumber(p.id) === jidNumber(senderId)
    );
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `𝗕𝗮𝗱 𝗪𝗼𝗿𝗱 𝗗𝗲𝗹𝗲𝘁𝗲𝗱 ⚠️\n@${senderId.split('@')[0]} 𝗗𝗼𝗻𝘁 𝗦𝗲𝗻𝗱 𝗔𝗴𝗮𝗶𝗻 ❗\n> 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 : 𝗠𝘂𝘇𝗮𝗺𝗶𝗹-𝗫𝗗`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `𝗕𝗮𝗱 𝗪𝗼𝗿𝗱 𝗗𝗲𝘁𝗲𝗰𝘁𝗲𝗱 ⚠️\n@${senderId.split('@')[0]} 𝗬𝗼𝘂 𝗛𝗮𝘃𝗲 𝗕𝗲𝗲𝗻 𝗥𝗲𝗺𝗼𝘃𝗲𝗱 ✅\n> 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 : 𝗠𝘂𝘇𝗮𝗺𝗶𝗹-𝗫𝗗`,
                    mentions: [senderId]
                });
            } catch (error) {
        ...
        case 'warn':
            const warningCount = await incrementWarningCount(chatId, senderId, sock);
            if (warningCount >= 3) {
                try {
                    await sock.groupParticipantsUpdate(chatId, [senderId], 'remove');
                ...
                await sock.sendMessage(chatId, {
                    text: `𝗕𝗮𝗱 𝗪𝗼𝗿𝗱 𝗗𝗲𝘁𝗲𝗰𝘁𝗲𝗱 ⚠️\n@${senderId.split('@')[0]} 𝗪𝗮𝗿𝗻𝗶𝗻𝗴 ${warningCount}/3 𝗗𝗼𝗻𝘁 𝗦𝗲𝗻𝗱 𝗔𝗴𝗮𝗶𝗻 𝗢𝘁𝗵𝗲𝗿𝘄𝗶𝘀𝗲 𝗬𝗼𝘂 𝗪𝗶𝗹𝗹 𝗕𝗲 𝗥𝗲𝗺𝗼𝘃𝗲𝗱 ❗\n> 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 : 𝗠𝘂𝘇𝗮𝗺𝗶𝗹-𝗫𝗗`,
                    mentions: [senderId]
                });
            }
            break;
