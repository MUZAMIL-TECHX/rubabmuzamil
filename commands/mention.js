...
function loadState() {
	try {
        ...
        // Ignore paths from older builds because only temp media is supported now.
        if (state && typeof state.assetPath === 'string' && !state.assetPath.startsWith('temp/')) {
...
// Send custom media or default fallback
		if (!state.assetPath) {
...
async function setMentionCommand(sock, chatId, message, isOwner) {
    ...
    // Remove previous custom media so only one mention response file is kept.
    ...
    try {
        ...
        if (stateBefore.assetPath && stateBefore.assetPath.startsWith('temp/')) {
        ...
        console.warn('cleanup previous mention media failed:', e?.message || e);
    }

    // Save into temp (the project no longer needs an assets folder).
	...
	const state = loadState();
state.assetPath = path.join('temp', outName);
	state.type = type;
